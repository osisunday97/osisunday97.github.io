...
app.get('/api', (req, res) => {
    ...
    fetch(baseURL)
        .then((r) => r.json())
        .then((data) => {
            // Filtering to get only INST courses
            data = data.filter(each_course => each_course.course_id.includes('INST'))
                //console.log(data);

            // Return list with each item in the format course_id: name
            var newdata = new Object;
            for (i = 0; i < data.length; i++) {
                newdata[data[i].course_id] = data[i].name;
            }
            console.log(newdata)
